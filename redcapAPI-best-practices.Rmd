...
unlockREDCap(c(test_conn    = 'TestRedcapAPI', # REDCap project 1
             ...
             passwordFUN  = rstudioapi::askForPassword)  # RStudio+MacOS
```
...
Unfortunately, at present the package `getPass` is broken on MacOS when using RStudio and pressing the 'Knit' button, and the alternative package `askpass` doesn't work on any platform in this case. If one wishes to use RStudio and `knitr` on MacOS then the 
`rstudioapi` package should be installed and `passwordFUN=rstudioapi::askForPassword` should
be specified in the call.

If one is not running from RStudio+MacOS the `passwordFUN` argument is not required
...
reason, we advise changing ones processes to utilize `castForImport` prior
to importing data. 
...
exclusively within each row of the data. Thus, if ones branching logic 
...
Use `restoreProject` to create one's own test database and `purgeProject` to purge
the data and metadata from the REDCap project.

```{r, eval = FALSE}
purgeProject(rcon, records = TRUE)

load(file.path(path.package('redcapAPI'), "extdata", "testingBranchingLogic", "TestingBranchingLogic.Rdata"))
