...
of power into the hands of those wishing to collect data, from surveys to 
...
[R](https://www.r-project.org/) being a useful tool for this purpose, the department of
Biostatistics at Vanderbilt University Medical Center has provided the
community with the package [`redcapAPI`](https://cran.r-project.org/package=redcapAPI) to
...
with where the REDCap project was today. The original package began to just
expose the raw API directly into R. The common needs of users propagated via snippets of R code. To address the true needs of the user, a major refactor
...
the API_KEY is exposed to the world. It's a very easy mistake to make. The risk of exposure of any plain text file in a directory is high. The problem
is so rampant that at one point there was a website, just scanning `github.com` for API_KEYS and posting them on a rolling kiosk and exposures were occurring every couple seconds. Many of these were for other APIs, but the risk of exposure through an inadvertent commit cannot be understated.
...
greatly reduces the risk of exposure. It has tools to facilitate a transfer of code to an automated environment as well. `?unlockREDcap` for those details. 
...
#### MacOS and Password Prompts

Unfortunately, at present `askpass` and `getPass` are broken on MacOS when using RStudio and pressing the 'Knit' button. Password
prompts will return `NULL` and never query the user. A workaround for MacOS users is to execute the above snippet from the console to configure the crypto locker, and then modify their `Rmd` to use parameters for the password. 

````{verbatin}
---
title: "example MacOS w/ Params"
output: html_document
params: 
  password:
    label: "Enter Password for API_KEYs"
    input: password
    value: ""
---
...
options(keyring_backend=keyring::backend_file) # For MacOS
...
unlockREDCap(c(test_conn    = 'TestRedcapAPI', # REDCap project 1
             ...
             envir        = globalenv(),
             url          = 'https://redcap.vanderbilt.edu/api/',
             passwordFUN  = function(...) params$password) 
```
````
...
More details on keyring management are in the `keyring` package. One useful function if one forgets the password is to delete it and try again, `keyring::keyring_delete("API_KEYs")`

Once again per our design goals, our choices and recommendations do not limit the user. If you have your own system of API_KEY management, one can still open a connection directly using `redcapConnection`. 
...
The connection objects are a much richer object than the older version of the library. During a lot of REDCap 
...
of the package gets this for **free** and by specifying `retries=10` it will try up to 30 minutes per call if necessary--allowing for downtime under this to not affect the generation of a report. This is important on automated systems that require reliabilily and can wait.  

## `exportRecordsTyped`

`exportRecords`, `redcapFactor` and `redcapFlipFactor` still exist in the
library but are deprecated. `exportRecordsTyped` is the preferred way forward. These functions will not longer be updated. 
...
The `widerRepeated` function converts a data frame into a short and wide one and ignore any data.frame that is not a repeated_instrument (this allows for post processing pipelining). 
...
are the same. If this is not the case it will erro
