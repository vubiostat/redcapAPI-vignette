...
prompts will return `NULL` and never query the user. A workaround for MacOS users is to execute the above snippet from the console to configure the crypto locker, and then modify their `Rmd` to use parameters for the password. From RStudio one now uses `Knit -> Knit with Parameters` to trigger the document generation.
